...
const MenuCard = ({ id, name, rating, price, image }) => {
    ...
    <StyledWrapper>
      <div className="card">
        ...
        <div className="card-footer">
          <span className="text-title price-btn">₹{price}</span>
          <span>
            {" "}
            {rating} <FontAwesomeIcon icon={faStar} color="#e6d300" />
          </span>
          <div className="card-button" onClick={() => handleCart(id)}>
...
const StyledWrapper = styled.div`
  .card {
    ...
    position: relative;
    overflow: visible;
    box-shadow: 6px 7px 7px rgba(0, 0, 0, 0.12), 0 4px 7px rgba(0, 0, 0, 0.24);
    border-radius: 12px;
    transition: 0.3s ease-in;
  }

  .card:hover {
    box-shadow: 0px 0px 20px 4px var(--bg3);
  }
